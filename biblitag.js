(function($) {
  var BIBLITAG = BIBLITAG || (function () {
    /**
     * Constructor.
     * @param translation
    ...
    Biblitag.prototype.parse = function (s) {
      ...
      for (var i = 0; i < pe.length; i++) {
        ...
        //   if one was matched.
        var toReplace = this.s.substring(pe[i].indices[0], pe[i].indices[1] - (pe[i].translations[0].length ?
                                                                               pe[i].translations[0].length + 1 :
                                                                               0)) + (pe[i].translations[0] ?
                                                                                      ' ' + pe[i].translations[0] :
                                                                                      '');
        // The Bible's version (translation).
        var version = pe[i].translations[0] ? pe[i].translations[0] :
    ...
    return {
      init: function (translation, provider, target) {
        $(window).load(function () {
          var bt = new Biblitag(translation, provider);
          target = target || 'body';
          $(target).html(bt.parse($(target).html()));
        });
      }
    }
  }());
